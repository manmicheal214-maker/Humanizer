...
"""${rewritten}"""`;
        ...
        // Voice pass: adds natural editorial personality rather than more\n        // constraint-satisfaction, since the rewrite and de-AI passes above\n        // already handle statistical AI-signal reduction. This targets tone\n        // instead.\n        try {\n          const voicePrompt = `You are adding natural human voice and personality to a sports article draft, without changing any facts.\n\nRead the draft below and revise it one more time, focusing specifically on:\n- Occasional brief editorial asides or opinion phrases a real sports journalist would naturally include (e.g. \"Make no mistake,\", \"It's worth remembering that,\", \"For all the talk of...\", \"There's no understating how...\"). Use these SPARINGLY — at most 3-4 times across the whole piece, never in every paragraph.\n- Varying how paragraphs and sections open — avoid starting multiple paragraphs with the same subject or sentence structure.\n- Writing with the confident, opinionated voice of a specific beat reporter, not a neutral summarizer.\n\nSTRICT RULES:\n- Do not change any facts, numbers, names, dates, or statistics.\n- Do not invent new information, quotes, or claims.\n- Preserve all \"## \" headers exactly as they are — do not remove, reword, or merge them into surrounding text.\n- Preserve bulleted list items exactly as separate lines — do not merge them into prose.\n- Preserve short, standalone dramatic sentences as-is.\n- Return ONLY the revised text. No preamble, no explanation, no meta-commentary.\n\nDRAFT:\n\"\"\"${rewritten}\"\"\"`;\n\n          const voiceResult = await callGeminiWorker({\n            prompt: voicePrompt,\n            apiKey: env.GEMINI_API_KEY,\n            model: env.GEMINI_MODEL || CONFIG.DEFAULT_MODEL,\n            temperature: 0.85\n          });\n          let voiced = voiceResult.text.trim();\n          for (const re of prefixes) {\n            if (re.test(voiced)) voiced = voiced.replace(re, \"\").replace(/^[\\s:—-]+/, \"\").trim();\n          }\n          voiced = stripMarkdownFences(voiced);\n          if (voiced) {\n            rewritten = voiced;\n          }\n        } catch (voiceErr) {\n          console.warn(\"Voice pass failed, keeping prior draft:\", voiceErr.message);\n        }\n\n        rewritten = applyDeterministicHumanization(rewritten);
